...
# Fetches raw results for one query: cache hit or scrape then cache; content_type in key for IG only.
async def fetch_one_query(q: Query) -> list[dict[str, Any]]:
    content_type = q.content_type if q.platform == "instagram" else ""
