...
func (s *ChatService) HandleChat(ctx context.Context, req models.ChatRequest) (*models.ChatResponse, error) {
	...
	if s.rateLimitRepo != nil {
		key := fmt.Sprintf("rl:chat:%s:%s", userID, time.Now().UTC().Format("2006-01-02"))
	...
	if s.chatRepo != nil {
		_ = s.chatRepo.SaveMessage(ctx, userID, "user", req.Message)
		_ = s.chatRepo.SaveMessage(ctx, userID, "agent", agentResp.ChatResponse)
	}

	// After every 5th user chat, trigger a background preferences update.
	if s.preferencesEnabled && s.chatRepo != nil {
		if count, err := s.chatRepo.CountMessages(ctx, userID); err == nil && count > 0 && count%5 == 0 {
			go s.updatePreferencesInBackground(userID, profile.Preferences)
		}
	}

	if !agentResp.NeedsNewContent {
		return result, nil
	}
	...
	// NOTE: Content generation can be slow. We return the chat response immediately
	// and let the frontend fetch new content (e.g. via GET /api/content) using
	// `needs_new_content` as the trigger/animation flag.
	return result, nil
}
